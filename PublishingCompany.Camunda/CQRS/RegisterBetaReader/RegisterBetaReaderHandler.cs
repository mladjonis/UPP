...
{
    public class RegisterBetaReaderHandler : IRequestHandler<RegisterBetaReaderRequest, RegisterBetaReaderResponse>
    {
        private readonly IFormSubmitDtoMapper _formMapper;
        private readonly BpmnService _bpmnService;

        public RegisterBetaReaderHandler(IFormSubmitDtoMapper formMapper, BpmnService bpmnService)
        {
            this._bpmnService = bpmnService;
            this._formMapper = formMapper;
        }

        public async Task<RegisterBetaReaderResponse> Handle(RegisterBetaReaderRequest request, CancellationToken cancellationToken)
        {
            var registerUserResponse = new RegisterBetaReaderResponse() { ProcessInstanceId = request.ProcessInstanceId };
            try
            {
                ...
                await _bpmnService.SetProcessVariableByProcessInstanceId("readerRegistrationValuesGenres", request.ProcessInstanceId, request.SubmitFields);
            }
            catch (Exception e)
            {
                //logger potencijalno
