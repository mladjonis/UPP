...
{
    [HandlerTopics("Topic_NotifyPlagiarismProposalIsPlagiarismUserHandler", LockDuration = 10_000)]
    public class NotifyPlagiarismProposalIsPlagiarismUserHandler : ExternalTaskHandler
    {
        private readonly BpmnService _bpmnService;
        private readonly IUnitOfWork _unitOfWork;
        private readonly IEmailService _emailService;

        public NotifyPlagiarismProposalIsPlagiarismUserHandler(BpmnService bpmnService, IUnitOfWork unitOfWork, IEmailService emailService)
        {
            ...
            try
            {
                ...
                if (writer == null || book == null || plagiarismProposalWriter == null)
                {
                    return new CompleteResult()
                    {
                        ...
                        {
                            ["NotifyPlagiarismProposalIsPlagiarismUserHandler"] = new Variable("Error something went wrong writer or book or proposal writer was not found", VariableType.String)
                        }
                    };
                }
                await _emailService.SendAsync(plagiarismProposalWriter.Email, "Book was plagiarism", $"Book that you proposed ${bookName} by ${writerName} was plagiarism. Thank you for submitting.");
            }
            catch (Exception e)
            {
                return new CompleteResult()
                {
                    Variables = new Dictionary<string, Variable>
                    {
                        ["NotifyPlagiarismProposalIsPlagiarismUserHandler"] = new Variable(e.Message, VariableType.String)
                    }
                };
            }
            return new CompleteResult() { };
